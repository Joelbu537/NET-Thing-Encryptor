# Videoplayer ab Version 4.2.0

## Umfang

Version 4.2.0 integriert die Videowiedergabe in den kanonischen Avalonia-Client für Windows und Android. Sie verwendet `LibVLCSharp` 3.10.1 und die jeweilige plattformgebundene VideoLAN-Laufzeit. Android bindet `VideoLAN.LibVLC.Android` 3.7.0-beta ein. Audio bleibt vorerst ohne internes Wiedergabebackend und kann weiterhin exportiert werden.
...
|---|---|
| Windows | `LibVLCSharp.Avalonia.VideoView` im eigenen Dokumentfenster |
| Android | native Videoansicht, über Avalonias `NativeControlHost` in die Dokumentansicht eingebettet; `VideoLAN.LibVLC.Android` 3.7.0-beta für `arm64-v8a` und `x86_64` |
| Linux/macOS | Desktop-`VideoView`; systemweite LibVLC-Laufzeit erforderlich und noch nicht verifiziert |

Eine Wiedergabesitzung besitzt genau einen LibVLC-Player und genau einen entschlüsselten Eingabestrom. Fenster- beziehungsweise Ansichtswechsel erzeugen keine zweite entschlüsselte Kopie. Beim Schließen der Dokumentansicht oder beim Sperren des Tresors wird die Sitzung vollständig beendet und die native Videoansicht getrennt.
...
Der Player bietet sichtbare Schaltflächen für Play/Pause sowie Sprünge um zehn Sekunden und eine seekbare Zeitleiste. Die Zeitposition wird während der Wiedergabe aktualisiert und kann per Zeiger oder Touch geändert werden.
...
Das Öffnen eines Videos übernimmt den entschlüsselten Puffer in die Wiedergabesitzung. Schlägt die Initialisierung fehl, wird der Puffer unmittelbar bereinigt und eine verständliche Fehlermeldung angezeigt. Nach erfolgreichem Start gilt diese Reihenfolge beim Schließen:
...
- Windows 10 und Windows 11: separates Videofenster, Seitenverhältnis, Größenänderung, Play/Pause, Zeitleiste, Schaltflächen und alle Tastenkürzel;
- Windows: Fenster während laufender und pausierter Wiedergabe schließen und anschließend dasselbe sowie ein anderes Video erneut öffnen;
- Windows: automatische Sperre auslösen und prüfen, dass Fenster, Wiedergabe und Ton beendet werden;
- Android 12 oder neuer auf realer `arm64-v8a`-Hardware: native Darstellung, Rotation beziehungsweise Größenänderung, Touchsteuerung, Seek, wiederholtes Öffnen, App-Hintergrund und automatische Sperre;
